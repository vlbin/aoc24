import { range } from '@fp-lib/array';
import { size } from '@fp-lib/coordinates';
import { coords } from '@fp-lib/parsing';
import type { Coordinate } from '@fp-lib/types';
            ...
            .flatMap((locations) => {
                ...
                return pairs
                    .flatMap(([a, b]) => {
                        ...
                        return [
                            ...range(0, Math.max(nCols, nRows)).map((i) => [a[0] - rowDiff * i, a[1] - colDiff * i]),
                        ];
                    })
                    .filter(([r, c]) => {
                        return r >= 0 && c >= 0 && r < nRows && c < nCols;
